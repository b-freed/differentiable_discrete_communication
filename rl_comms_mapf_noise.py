...
class Worker:
    ...
    def train(self, rollout, sess, gamma, bootstrap_values):
       ...
       #14: train_val
        ...
        valids = rollout[:,11]
        pred_blockings = rollout[:,12]
        blockings = rollout[:,13]
        # train_value = rollout[:,14]
        ...
        feed_dict = {
            ...
            self.local_AC.msg_input:np.stack(msg_mats[:len(action_advantages)]),
            self.local_AC.msg_binary:np.stack(msgs_binary[:len(action_advantages)]),
            self.local_AC.actions:actions[:len(action_advantages)],
            self.local_AC.actions_other:actions_other[:len(action_advantages)],
            self.local_AC.train_valid:np.stack(valids[:len(action_advantages)]),
            self.local_AC.action_advantages:action_advantages,
            self.local_AC.msg_advantages:msg_advantages,
            self.local_AC.target_blockings:blockings[:len(action_advantages)]
        }
        
        v_l,p_l,msg_l,valid_l,e_l,e_msg,gn_a,gn_c,vn_a,vn_c,b_l,_,_ = sess.run([
        ...
        return v_l/len(rollout), p_l/len(rollout), msg_l/len(rollout), valid_l/len(rollout), e_l/len(rollout), e_msg/len(rollout), b_l/len(rollout), gn_a, gn_c, vn_a, vn_c
    ...
    def choose_action_msg(self, s_actor, s_critic, validActions, training):
        ...
        a_dist, P_msg,pred_blocking, msg_entropy = sess.run([self.local_AC.policy,
                                                self.local_AC.P_msg,
        ...
        return a,msg_binary,v,pred_blocking,invalid
...
#                 validActions, on_goal = self.env._reset(self.agentID)
                ...
                while (not self.env.finished): # Give me something!
                    ...
                    a,msg_binary,v,pred_blocking,invalid = self.choose_action_msg(s_actor,s_critic, validActions, TRAINING)
                    ...
                    episode_buffer.append([s_actor[0],s_actor[1],s_critic,a,action_buffer[self.metaAgentID][other_agentID - 1],msg_binary,r,s1_actor,d,v,value_buffer[self.metaAgentID][other_agentID - 1],train_valid,pred_blocking,int(blocking)])
...
#                         s1 = s  #Oh yeah!! We are done, we did it!!!
                    ...
                    if TRAINING and (len(episode_buffer) % EXPERIENCE_BUFFER_SIZE == 0 or d):
                        ...
                        if not d:
                            ...
                            #a,msg_1hot,v,train_val,pred_blocking,invalid
                            _, _,v,_,_ = self.choose_action_msg(s_actor, s_critic, validActions, TRAINING)
                            #place this state's value in the value buffer
                            value_buffer[self.metaAgentID][self.agentID - 1] = v
                        ...
                        if (episode_count-EPISODE_START) < NUM_BUFFERS:
                            ...
                            while tmp.shape[0] == 0:
                                i_rand = np.random.randint(NUM_BUFFERS)
                                tmp = np.array(episode_buffers[i_rand])
                        #v_l,p_l, msg_l, valid_l, e_l, e_msg, b_l, gn_a, gn_c, vn_a, vn_c
                        v_l, p_l, msg_l, valid_l, e_l, e_msg, b_l, gn_a, gn_c, vn_a, vn_c = self.train(episode_buffers[i_rand],sess,gamma,s1Values[i_rand])
...
#                    saveGIF &= (episode_step_count < max_episode_length)
                    ...
                    if episode_count % SUMMARY_WINDOW == 0:
                        ...
                        summary.value.add(tag='Losses/Message Loss', simple_value=e_l)
...
load_model             = False
RESET_TRAINER          = False
model_path             = './model_rl_comms_toy_noise_AAAI'
gifs_path              = './gifs_rl_comms_toy_noise_AAAI'
train_path             = 'train_rl_comms_toy_noise_AAAI'
...
with tf.device("/gpu:0"):
    master_network = ACNet(GLOBAL_NET_SCOPE,a_size,None,None,False,GRID_SIZE,GLOBAL_NET_SCOPE) # Generate global network
