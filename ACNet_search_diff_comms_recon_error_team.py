...
msg_length             = 100 #number of elements in message
N_CHANNELS_ACTOR       = 5
N_CHANNELS_CRITIC      = 5
ENV_DIMENSION          = 2
...
class ACNet:
    ...
    def _build_net(self,actor_inputs,critic_inputs,recon_error, a_size,GRID_SIZE):
        ...
        with tf.variable_scope('actor'):
            ...
            state_in = tf.concat([self.rnn_state_in, self.msgs_in], axis = 1) # "state_in corresponds to the rnn state for each agent combined with the messages to each agent from all other agents"
...
class RoutingRNN(tf.nn.rnn_cell.RNNCell):
    ...
    def state_size(self):
        ...
        return self._hidden_state_size + self._msg_input_length
    ...
    def call(self, inputs_and_recon_error, state_in):
        ...
        inputs = inputs_and_recon_error[:,:-self._msg_input_length]
        # final self._msg_input_length of inputs_and_recon_error is recon error
        recon_error = inputs_and_recon_error[:,-self._msg_input_length:]
        ...
        pre_msg          = layers.fully_connected(inputs=h1, num_outputs=self._msg_length, activation_fn=tf.nn.sigmoid)
        msgs_out         = tf.reverse(pre_msg, [0]) + recon_error # These are messages TO each agent, with reconstruction error added
        
        # combine hidden_state_out and msgs_out to create state_out
        state_out        = tf.concat([hidden_state_out,msgs_out],axis = 1)
