...
class MAPFEnv(gym.Env):
	...
	def getRelativePos(self):
		#same as getPositions but relative to each agent from 1,2,3...
		rel_result = []
		for i in range(1, self.num_agents+1): 
			rel_result.append([])
			self_pos = self.world.getPos(i)
			#start appending same team rel pos
			for j in range(1, self.num_agents+1):
				if j % 2 == i % 2 and i != j:
					rel_result[i-1] = np.concatenate((rel_result[i-1], np.subtract(self_pos, self.world.getPos(j))))

			for k in range(1, self.num_agents+1):
				if k % 2 != i % 2:
					rel_result[i-1] = np.concatenate((rel_result[i-1], np.subtract(self_pos, self.world.getPos(k))))
		
		#rel_result = np.reshape(rel_result, (1, self.num_agents, len(self.world.getPos(1))))
		return rel_result
...
#         size=np.random.triangular(self.SIZE[0],self.SIZE[0],self.SIZE[1])
	...
	# Not being used at this time although we may want to use it in the future
	...
	# Resets environment
	def _reset(self,world0=None,goal_locs=None,goal_types=None,blank_world=False):
		self.finished = False
		self.mutex.acquire()
